...
async def encrypt(
    ...
    passfile: UploadFile = File(None)
):
    try:
        validate_passphrase(passphrase, passfile)
        validate_file_for_encryption(file)
        
        passphrase_input = passfile if passfile else passphrase
        key = await derive_key_from_passphrase(passphrase_input)
