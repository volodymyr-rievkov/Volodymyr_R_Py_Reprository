...
from .md5_utils import md5_from_file, md5_from_string
...
async def derive_key_from_passphrase(passphrase: str  | UploadFile, key_bits: int = KEY_BITS) -> bytes:
    if isinstance(passphrase, str):
        h = md5_from_string(passphrase, hex=False)  
    elif hasattr(passphrase, "read"):
        h = await md5_from_file(passphrase, hex=False)  
    else:
        raise TypeError("passphrase must be str or UploadFile")
    ...
    if key_bits == 64:
        ...
        h2 = md5_from_string(h[:16], hex=False)
        return h + h2
    else:
        raise ValueError("Unsupported key length")


def validate_passphrase(passphrase: Optional[str], passfile: Optional[UploadFile]) -> None:
    if not passphrase and not passfile:
